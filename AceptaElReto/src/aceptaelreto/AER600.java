...
public class AER600 {
    ...
    static boolean hayPuntosSilla(int[][] m) {
        int[][] minMaxFilas = estadisticasFilas(m);
        int[][] minMaxColumnas = estadisticasColumnas(m);
        
        for (int f = 0; f < m.length; f++) 
            for (int c = 0; c < m[f].length; c++) 
                if (m[f][c] == minMaxFilas[f][0] &&
                    m[f][c] == minMaxColumnas[1][c]
                        
                    ||
                        
                    m[f][c] == minMaxFilas[f][1] &&
                    m[f][c] == minMaxColumnas[0][c]    
                    )
                    return true;
    ...
    public static void main(String[] args) {
        ...
        while ((filas = in.nextInt()) != 0){
            ...
            System.out.println(hayPuntosSilla(m)? "SI" : "NO");
        }
    }
}